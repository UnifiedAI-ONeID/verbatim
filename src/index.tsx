...
// FIX: Updated Firebase imports to use the v9 compat libraries, which provide the v8 namespaced API surface.
...
// IMPORTANT: The main analysis is now intended to be done via a secure Cloud Function.
// We keep a client-side instance for non-sensitive, UX-enhancing features like 'Take Action'.
...
// --- i18n Translations (omitted for brevity, remains the same) ---
const translations = {
    en: {
        title: 'Verbatim',
...
const App = () => {
    ...
    const [isSaving, setIsSaving] = useState(false); // New state for post-recording save
    ...
    useEffect(() => {
        // FIX: Updated onAuthStateChanged and Firestore calls to v8 syntax.
    ...
    useEffect(() => {
        ...
        // FIX: Updated Firestore query to v8 syntax.
        const sessionsColRef = db.collection('users').doc(user.uid).collection('sessions');
        const q = sessionsColRef.orderBy('metadata.date', 'desc');
        
        const unsubscribeSessions = q.onSnapshot((snapshot) => {
    ...
    const signInWithGoogle = async () => {
        // FIX: Updated GoogleAuthProvider and signInWithPopup to v8 syntax.
    ...
    const handleSignOut = async () => {
        try {
            // FIX: Updated signOut to v8 syntax.
            await auth.signOut();
    ...
    const handleDeviceSelected = async (deviceId: string) => {
        ...
        // Create a preliminary session doc in Firestore to get an ID
        const newSessionId = `session_${Date.now()}`;
        // FIX: Updated Firestore doc reference to v8 syntax.
        const sessionDocRef = db.collection('users').doc(user.uid).collection('sessions').doc(newSessionId);
        ...
        try {
            ...
            mediaRecorderRef.current.onstop = async () => {
                ...
                try {
                    ...
                    // FIX: Updated Firebase Storage ref and upload method to v8 syntax.
                    const storageRef = storage.ref(`recordings/${user.uid}/${newSessionId}.webm`);
                    ...
                    // 3. Update the session document with metadata
                    ...
                    // FIX: Updated Firestore setDoc to v8 syntax.
                    await sessionDocRef.set(preliminarySession);
                    setSelectedSession({ ...preliminarySession, id: newSessionId, results: { transcript: '', summary: '', actionItems: [] }, speakers: {} });

                    // 4. Trigger the Cloud Function for analysis
                    // IMPORTANT: You need to deploy a Cloud Function named 'analyzeAudio'.
                    // See the comment block below for the required backend code.
                    // FIX: Updated httpsCallable to v8 syntax.
                    ...
                    // FIX: Updated deleteDoc to v8 syntax.
                    await sessionDocRef.delete(); // Clean up failed session
                } finally {
                    setIsSaving(false);
                }
    ...
    const handleDeleteSession = async (sessionId: string) => {
        if (user && window.confirm(t.deleteConfirmation)) {
            // FIX: Updated Firestore delete to v8 syntax.
            const sessionDocRef = db.collection('users').doc(user.uid).collection('sessions').doc(sessionId);
            await sessionDocRef.delete();
            // Storage file deletion can be handled by a Cloud Function lifecycle trigger for robustness
    ...
    const handleRenameSpeaker = async (sessionId: string, speakerId: string, newName: string) => {
        ...
        if (user && sessionToUpdate && newName.trim()) {
            ...
            // FIX: Updated Firestore update to v8 syntax.
            const sessionDocRef = db.collection('users').doc(user.uid).collection('sessions').doc(sessionId);
            await sessionDocRef.update({ speakers: updatedSpeakers });
        }
    ...
    const generateMarkdown = (session: Session) => {
        if (session.status !== 'completed') return "Session is still processing.";
...
${transcriptText}
    ...
    const determineAction = async (actionItem: string, session: Session): Promise<ActionModalData> => {
        const createCalendarEvent: FunctionDeclaration = { name: 'create_calendar_event', description: 'Creates a Google Calendar event.', parameters: { type: Type.OBJECT, properties: { title: { type: Type.STRING }, description: { type: Type.STRING }, date: { type: Type.STRING }, time: { type: Type.STRING } }, required: ['title', 'date', 'time'] } };
        const draftEmail: FunctionDeclaration = { name: 'draft_email', description: 'Drafts an email.', parameters: { type: Type.OBJECT, properties: { to: { type: Type.STRING }, subject: { type: Type.STRING }, body: { type: Type.STRING } }, required: ['to', 'subject', 'body'] } };
        const initiatePhoneCall: FunctionDeclaration = { name: 'initiate_phone_call', description: 'Initiates a phone call.', parameters: { type: Type.OBJECT, properties: { phoneNumber: { type: Type.STRING }, reason: { type: Type.STRING } }, required: ['phoneNumber'] } };
        const createDocument: FunctionDeclaration = { name: 'create_document', description: 'Creates a text document.', parameters: { type: Type.OBJECT, properties: { title: { type: Type.STRING }, content: { type: Type.STRING } }, required: ['title', 'content'] } };
        const draftInvoiceEmail: FunctionDeclaration = { name: 'draft_invoice_email', description: 'Drafts an email to send an invoice.', parameters: { type: Type.OBJECT, properties: { to: { type: Type.STRING }, recipientName: { type: Type.STRING }, subject: { type: Type.STRING }, amount: { type: Type.NUMBER }, currencySymbol: { type: Type.STRING }, itemDescription: { type: Type.STRING } }, required: ['to', 'recipientName', 'subject', 'amount', 'currencySymbol', 'itemDescription'] } };
        ...
        try {
            ...
            // FIX: Simplified the 'contents' property for a single text prompt.
        ...
        if (isEditing) {
            return (
                <input
                    ...
                    onKeyDown={(e) => {
                        if (e.key === 'Enter') handleRenameSpeaker(sessionId, speakerId, (e.target as HTMLInputElement).value);
...
// FIX: Changed User type to firebase.User for v8 compatibility.
const RecordScreen = ({ user, isRecording, recordingTime, onStart, onStop, onTogglePip, keepAwake, onKeepAwakeChange, onSignIn }: { user: firebase.User | null; isRecording: boolean, recordingTime: number, onStart: () => void, onStop: () => void, onTogglePip: () => void, keepAwake: boolean, onKeepAwakeChange: (value: boolean) => void, onSignIn: () => void }) => {
...
// FIX: Changed User type to firebase.User for v8 compatibility.
const SessionList = ({ sessions, onSelectSession, onDeleteSession, searchQuery, onSearchChange, user, onShowFaq, onSignIn, onSignOut }: { sessions: Session[]; onSelectSession: (s: Session) => void; onDeleteSession: (id: string) => void; searchQuery: string; onSearchChange: (e: React.ChangeEvent<HTMLInputElement>) => void; user: firebase.User | null; onShowFaq: () => void; onSignIn: () => void; onSignOut: () => void; }) => (
    <div className="page-container">
        <div className="page-header">
             <h1 className="page-title">{t.sessions}</h1>
             <div className="header-actions">
                 {user && <span className="welcome-user">{t.welcomeUser.replace('{name}', user.displayName?.split(' ')[0] || '')}</span>}
                 {user && <button onClick={onSignOut} className="signout-button" aria-label={t.signOut}>{t.signOut}</button>}
                        ...
                        {sessions.map(session => (
                            <li key={session.id} className="session-item" onClick={() => onSelectSession(session)} tabIndex={0} onKeyDown={e => e.key === 'Enter' && onSelectSession(session)}>
                                <div className="session-item-content">
                                    ...
                                    {session.status === 'completed' && <p className="summary-preview">{(session.results.summary || '').slice(0, 100)}...</p>}
                                    {session.status === 'processing' && <div className="processing-indicator"><div className="spinner-small"></div> {t.processing}</div>}
                                    {session.status === 'error' && <p className="error-text">{t.processingError}</p>}
                                </div>
        ...
        return (
            <div className="page-container session-detail">
                ...
                <div className="processing-view">
                    <LoadingModal text={t.processing} />
                    <p>This session is currently being analyzed. The results will appear here automatically when ready.</p>
                </div>
            </div>
        )
    }
         ...
         return (
            <div className="page-container session-detail">
                ...
                <div className="processing-view">
                    ...
                    <p>{session.error || t.processingError}</p>
                </div>
            </div>
        )
    }
...
// FIX: Changed User type to firebase.User for v8 compatibility.
const ActionModal = ({ data, onClose, user }: { data: ActionModalData; onClose: () => void; user: firebase.User | null }) => {
...
const globalStyles = `
    ...
    * {
    ...
    /* Session Detail */
    .session-detail-content, .processing-view {
        padding-top: 1rem;
    }
    .processing-view {
        text-align: center;
