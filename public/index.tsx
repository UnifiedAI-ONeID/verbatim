...
const translations = {
    ...
    es: {
        ...
        retryFailed: 'No se pudo iniciar el reintento.',
...
const LanguageContext = createContext<{ lang: Language; setLang: (lang: Language) => void; t: (typeof translations.en) & (typeof translations.es) & (typeof translations['zh-CN']) & (typeof translations['zh-TW']) }>({ lang: 'en', setLang: () => {}, t: translations.en });
...
const App = () => {
    ...
    const handleRetryAnalysis = async (sessionId: string) => {
        if (!user) return;
        setError(null);
        const sessionDocRef = doc(db, 'users', user.uid, 'sessions', sessionId);
        try {
            await updateDoc(sessionDocRef, { status: 'processing', error: null });

            // Optimistically update the UI
            const updatedSession = sessions.find(s => s.id === sessionId);
            if (updatedSession) setSelectedSession({ ...updatedSession, status: 'processing', error: undefined });

            const analyzeAudio = httpsCallable(functions, 'analyzeAudio');
            await analyzeAudio({ sessionId: sessionId, prompt: t.analysisPrompt });
        } catch (e) {
            setError(t.retryFailed);
            await updateDoc(sessionDocRef, { status: 'error', error: t.retryFailed });
        }
    };
    ...
    const renderContent = () => {
        if (selectedSession) return <SessionDetailView session={selectedSession} onBack={() => setSelectedSession(null)} onDelete={handleDeleteSession} onTakeAction={handleTakeAction} onUpdateSpeakerName={handleUpdateSpeakerName} editingSpeaker={editingSpeaker} setEditingSpeaker={setEditingSpeaker} onRetry={handleRetryAnalysis} />;
...
const SessionsListView = ({ sessions, onSelectSession, searchQuery, setSearchQuery }: any) => {
    ...
    const showErrorDetails = (e: React.MouseEvent, error: string) => {
    ...
    return (
        <div style={styles.detailView}>
            ...
            {session.status === 'completed' && session.results ? (
                ...
                <div style={{textAlign: 'center', padding: '20px', backgroundColor: 'var(--danger-bg-light)', borderRadius: '8px'}}>
                    <p style={{...styles.errorText, fontWeight: 'bold'}}>{t.processingError}</p>
                    <p style={{color: 'var(--text-secondary)'}}>{session.error}</p>
                    <button onClick={() => onRetry(session.id)} style={styles.primaryButton}>{t.retryAnalysis}</button>
                </div>
            )}
        </div>
    );
...
const styles: { [key: string]: CSSProperties } = {
    ...
    errorChip: { backgroundColor: 'var(--danger-bg)', color: 'var(--danger-text)', padding: '4px 8px', borderRadius: '12px', fontSize: '0.8rem', cursor: 'pointer' },
